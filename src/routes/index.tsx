...
function Index() {
  ...
  return (
    <div className="flex flex-col items-center min-h-[70vh] gap-8 max-w-2xl mx-auto transition-colors pb-12">
      ...
      <div className="w-full border-t border-zinc-200 dark:border-zinc-800 pt-8 flex flex-col items-center gap-6">
        ...
        <div className="grid grid-cols-2 gap-4 w-full bg-white dark:bg-zinc-900/50 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-zinc-700 dark:text-zinc-300">Background Color</label>
            <div className="flex gap-2">
              <input type="color" value={themeInput.backgroundColor} onChange={(e) => setThemeInput({...themeInput, backgroundColor: e.target.value})} className="w-8 h-8 rounded cursor-pointer border-0 p-0 bg-transparent" />
              <input type="text" value={themeInput.backgroundColor} onChange={(e) => setThemeInput({...themeInput, backgroundColor: e.target.value})} className="flex-1 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md px-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
            </div>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-zinc-700 dark:text-zinc-300">Text Color</label>
            <div className="flex gap-2">
              <input type="color" value={themeInput.textColor} onChange={(e) => setThemeInput({...themeInput, textColor: e.target.value})} className="w-8 h-8 rounded cursor-pointer border-0 p-0 bg-transparent" />
              <input type="text" value={themeInput.textColor} onChange={(e) => setThemeInput({...themeInput, textColor: e.target.value})} className="flex-1 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md px-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" value={themeInput.borderRadius} onChange={(e) => setThemeInput({...themeInput, borderRadius: e.target.value})} className="w-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md px-2 py-1.5 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
          </div>
      ...
      <AdaptiveFatalError isOpen={isFatalOpen} onOpenChange={setIsFatalOpen} title="Ocorreu um erro crítico" description="Nossos servidores falharam." />
    </div>
  )
}
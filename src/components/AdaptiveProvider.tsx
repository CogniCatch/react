...
export interface AdaptiveContextType {
	...
	captureAsyncError: (
		error: Error | unknown,
		options?: CaptureAsyncOptions,
	) => Promise<void>
}

export const AdaptiveContext = createContext<AdaptiveContextType | undefined>(
	undefined,
)
...
export function AdaptiveProvider({
	...
	const captureAsyncError = useCallback(
		async (rawError: Error | unknown, options?: CaptureAsyncOptions) => {
			const error =
			...
			if (apiKey === "sk_test_mock") {
				setTimeout(() => {
					toast.info("Mock Mode Analysis", {
						id: toastId,
						description:
							"This is a mocked async recovery because sk_test_mock was used.",
					})
				}, 1500)
				return
			}
			...
			try {
				...
				const saasEndpoint =
					apiUrl || "https://api.cognicatch.dev/v1/analyze-error"
				const browserLang =
					typeof navigator !== "undefined" ? navigator.language : "en-US"
				const finalLanguage = language || browserLang

				const finalHandleUserBilling =
					options?.handleUserBilling !== undefined
				...
				const response = await fetch(saasEndpoint, {
					...
					body: JSON.stringify({
						...
						stack: safeContext.componentName
							...
							: safeContext.errorCode || "Async Error",
				...
				if (aiResult) {
					...
					if (severity === "low") {
						toast.info(aiResult.title, {
							id: toastId,
							description: aiResult.description,
						})
					} else {
						toast.error(aiResult.title, {
							id: toastId,
							description: aiResult.description,
						})
					}
				...
				toast.error("Analysis Failed", {
					id: toastId,
					description:
						"We couldn't reach the AI servers. Please try again later.",
				})
			}
		},

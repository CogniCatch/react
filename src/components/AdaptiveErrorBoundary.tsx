...
const StaticFallback = ({ onRecover, theme }: StaticFallbackProps) => {
	...
	return (
		<div
			className={`p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm ${isDark ? "dark" : ""}`}
		>
...
export class AdaptiveErrorBoundary extends Component<Props, State> {
	...
	processAutoMode = async (error: Error, errorInfo: ErrorInfo) => {
				...
				if (this._isMounted) {
					this.setState({
						...
						aiData: {
							...
							description:
								"This is a mocked response because sk_test_mock was used.",
		...
		try {
			...
			const saasEndpoint =
				finalApiUrl || "https://api.cognicatch.dev/v1/analyze-error"
			...
			if (this._isMounted) {
				this.setState({
					...
					aiData: {
						...
						description:
							"We were unable to connect to the analytics servers at this time. Our team has already been notified.",
						actionLabel: "Reload Component",
					},
				})
			}
